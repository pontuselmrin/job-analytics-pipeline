...
DEFAULT_QUALITY_GATES_PATH = PROJECT_ROOT / "ops" / "harness" / "quality_gates.yaml"
...
def _effective_thresholds(org_abbrev: str, gates: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    defaults = dict(gates.get("defaults") or {})
    override = dict((gates.get("org_overrides") or {}).get(org_abbrev) or {})

    effective = dict(defaults)
    for key in ("min_jobs_per_org", "min_words", "max_words", "max_fetch_seconds_per_job", "allow_empty"):
        if key in override:
...
def validate_org_block(
    ...
    if org_block.get("scraper_error"):
        violations.append(f"{org_abbrev}: scraper_error present: {org_block['scraper_error']}")
    ...
    if expires_on:
        try:
            ...
            violations.append(f"{org_abbrev}: invalid override expires_on '{expires_on}'")
    ...
    if len(jobs) > 2 and len(successful) >= 3 and len(set(word_counts)) == 1:
        violations.append(f"{org_abbrev}: all successful descriptions share identical length ({word_counts[0]} words)")
...
def validate_postings_payload(payload: dict[str, Any], gates: dict[str, Any], today: date | None = None) -> list[str]:
    violations: list[str] = []
