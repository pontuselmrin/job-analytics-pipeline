...
@pytest.mark.batch
@pytest.mark.network
@pytest.mark.parametrize("org", BATCHES["B01"]["orgs"])
def test_batch_b01_contract(org):
    filepath, org_name, _is_pw = _find_scraper(org)
    jobs = _load_and_run(filepath)

    # Some target orgs are known intermittent; contract allows explicit empties.
    if org in {"AMLA", "UNFCCC"} and not jobs:
        return
